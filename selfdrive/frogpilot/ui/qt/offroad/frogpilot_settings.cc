...
FrogPilotSettingsWindow::FrogPilotSettingsWindow(SettingsWindow *parent) : QFrame(parent) {
  ...
  ButtonParamControl *togglePreset = new ButtonParamControl("TuningLevel", tr("Tuning Level"),
                                        ...
                                        togglePresets);
  int timeTo1FPHour = 1 - (paramsTracking.getInt("FrogPilotMinutes") / 60);
  int timeTo1OPHour = 1 - (params.getInt("openpilotMinutes") / 60);
  togglePreset->setEnabledButtons(3, timeTo1FPHour <= 0 || timeTo1OPHour <= 0);
